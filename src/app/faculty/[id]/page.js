  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Try to fetch faculty-courses, if endpoint doesn't exist, assume no courses assigned
        let assignedCourseIds = [];
        try {
          const facultyCoursesRes = await axios.get(`http://localhost:3001/faculty-courses?facultyId=${params.id}`);
          assignedCourseIds = facultyCoursesRes.data.map(fc => fc.courseId);
        } catch (error) {
          console.log('Faculty-courses endpoint not available:', error);
          // If the faculty has assignedCourses property, use that instead
          if (facultyRes.data.assignedCourses) {
            assignedCourseIds = facultyRes.data.assignedCourses.map(id => parseInt(id));
          }
        }

        // Filter courses based on assignments
        const assignedCourses = coursesRes.data.filter(course => 
